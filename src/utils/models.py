...
def inference_multitask_multiclass_classification_segmentation(
    ...
    for i, test_data in enumerate(test_loader):
        ...
        # Handle deep supervision correctly
            if isinstance(pred_class, list):
                pred_class_tensor = torch.mean(
                    torch.stack(pred_class, dim=0), dim=0
                )
            else:
                pred_class_tensor = pred_class

    # Get predicted class index correctly
        pred_class_idx = pred_class_tensor.argmax(dim=1).item()

    # If predicted normal → erase all tumor pixels
        if pred_class_idx == 2:
